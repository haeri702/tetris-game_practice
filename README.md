...
배경 그림은 이미지 파일이 아니라 **코드로 직접 그린 SVG**라서, 외부 파일 없이 `tetris.html` 하나만 있으면 동작합니다.

## 플레이 방법

`tetris.html` 파일을 다운로드해서 브라우저로 열면 바로 실행됩니다. 설치나 서버가 필요 없습니다.
...
```
tetris.html   게임 전체 (HTML + CSS + JavaScript)
README.md     이 문서
```

별도의 라이브러리나 빌드 도구를 쓰지 않았습니다. `tetris.html` 안에서 배경 그리기 코드와 게임 로직이 주석으로 구분되어 있습니다.
